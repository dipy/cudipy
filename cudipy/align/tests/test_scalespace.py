...
from cudipy.align.scalespace import IsotropicScaleSpace, ScaleSpace
    ...
    for test_class in [ScaleSpace, IsotropicScaleSpace]:
        for dim in [2, 3]:
            ...
            if test_class is ScaleSpace:
                ...
                sigmas = [3.0, 1.0, 0.0]
                ss = test_class(
                    original, factors, sigmas, grid2world, input_spacing
                )
...
def test_scale_space_exceptions():
    ...
    for level in range(3):
        ...
        cp.testing.assert_array_equal(zeros, z)
