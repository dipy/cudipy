...
from dipy.align import vector_fields as dipy_vfu
from dipy.data import get_fnames

from cudipy.align import floating
from cudipy.align import imwarp as imwarp
from cudipy.align import metrics as metrics
from cudipy.align.imwarp import DiffeomorphicMap


def test_mult_aff():
    r""" Test matrix multiplication using None as identity
    """
    A = cp.asarray([[1.0, 2.0], [3.0, 4.0]])
    B = cp.asarray([[2.0, 0.0], [0.0, 2.0]])

    C = imwarp.mult_aff(A, B)
    expected_mult = cp.asarray([[2.0, 4.0], [6.0, 8.0]])
    cp.testing.assert_array_almost_equal(C, expected_mult)

    C = imwarp.mult_aff(A, None)
    cp.testing.assert_array_almost_equal(C, A)

    C = imwarp.mult_aff(None, B)
    cp.testing.assert_array_almost_equal(C, B)
...
def test_diffeomorphic_map_2d():
    ...
    disp, assign = dipy_vfu.create_random_displacement_2d(
    ...
    disp = cp.asarray(disp)  # move displacement field to the GPU
    ...
    for type in [floating, cp.float64, cp.int64, cp.int32]:
        ...
        # precision errors)
        cp.testing.assert_array_almost_equal(warped, expected, decimal=5)
        ...
        # it is n.n.)
        cp.testing.assert_array_almost_equal(warped, expected)
    ...
    with pytest.raises(ValueError):
        diff_map.transform(moving_image, "linear")
    ...
    # info, but it's None...
    with pytest.raises(ValueError):
        diff_map.transform_inverse(moving_image, "linear")

    # We must provide, at least, the reference grid shape
    with pytest.raises(ValueError):
        imwarp.DiffeomorphicMap(2, None)

    # Verify that matrices are correctly interpreted from string
    non_array_obj = diff_map
    array_obj = cp.ones((3, 3))
    with pytest.raises(ValueError):
        diff_map.interpret_matrix("a different string")
    with pytest.raises(ValueError):
        diff_map.interpret_matrix(non_array_obj)
    assert diff_map.interpret_matrix("identity") is None
    assert diff_map.interpret_matrix(None) is None
    cp.testing.assert_array_equal(
        diff_map.interpret_matrix(array_obj), array_obj
    )


@pytest.mark.parametrize("ndim", [2, 3])
def test_diffeomorphic_map_simplification(ndim):
    ...
    if ndim == 2:
        ...
        trans_inv = np.linalg.inv(trans)
        scale = np.array([[1 * s, 0, 0], [0, 1 * s, 0], [0, 0, 1]])
        create_func = dipy_vfu.create_circle
        disp_func = dipy_vfu.create_harmonic_fields_2d
    elif ndim == 3:
        ...
        create_func = dipy_vfu.create_sphere
        disp_func = dipy_vfu.create_harmonic_fields_3d
    gt_affine = trans_inv.dot(scale.dot(trans))
    # Create the invertible displacement fields and the sphere
    radius = 16
    volume = create_func(*codomain_shape, radius)
    d, dinv = disp_func(*domain_shape, 0.3, 6)
    ...
    diff_map = imwarp.DiffeomorphicMap(
        ndim, domain_shape, R, domain_shape, D, codomain_shape, C, P
    )
    diff_map.forward = cp.asarray(d, dtype=floating)
    diff_map.backward = cp.asarray(dinv, dtype=floating)
    # Warp the volume to obtain the expected image
    expected = diff_map.transform(volume, "linear")

    # Simplify
    simplified = diff_map.get_simplified_transform()
    # warp the volume
    warped = simplified.transform(volume, "linear")
...
def test_get_direction_and_spacings():
    ...
    cp.testing.assert_array_almost_equal(direction, direction_gt)
    cp.testing.assert_array_almost_equal(spacings, spacings_gt)
...
def get_warped_stacked_image(image, nslices, b, m):
    ...
    d, dinv = dipy_vfu.create_harmonic_fields_2d(shape[0], shape[1], b, m)
    ...
    vol = cp.zeros(shape=image.shape + (nslices,))
    vol[..., zero_slices : (2 * zero_slices)] = image[..., None]
    wvol = cp.zeros(shape=image.shape + (nslices,))
...
def test_cc_2d():
    ...
    warped = mapping.transform(moving)
    starting_energy = cp.sum((static - moving) ** 2)
    final_energy = cp.sum((static - warped) ** 2)
    reduced = 1.0 - final_energy / starting_energy

    assert reduced > 0.9


@pytest.mark.parametrize("coord_axis", [0, -1])
def test_cc_3d(coord_axis):
    ...
    sigma_diff = 2.0
    radius = 2
    similarity_metric = metrics.CCMetric(
        3, sigma_diff, radius, coord_axis=coord_axis
    )
